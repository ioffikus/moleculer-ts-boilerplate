...
import { nodeID, transporter, env, log } from './global.config';
import validator from './utils/validator';

const isShowMetrics = () => env === 'stage' || env === 'developmemt';
 ...
 *
 * 	To overwrite broker’s deeply nested default options, which are
 *  not presented in "moleculer.config.ts",
 * 	via environment variables, use the `MOL_` prefix and double underscore `__`
 *  for nested properties in .env file.
...
const brokerConfig: BrokerOptions = {
  ...
  // More info: https://moleculer.services/docs/0.13/logging.html
  ...
  // More info: https://moleculer.services/docs/0.13/fault-tolerance.html#Retry
  retryPolicy: {
    // Enable feature
    enabled: false,
    // Count of retries
  ...
  // Limit of calling level.
  // If it reaches the limit, broker will throw an MaxCallLevelError error.
  ...
  // More info: https://moleculer.services/docs/0.13/metrics.html
  metrics: isShowMetrics(),
